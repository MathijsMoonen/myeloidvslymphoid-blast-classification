...
class Img_DataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split != "compute":  # Use compute if you only want the prediction results. if you do this, make sure you don't shuffle the data
            _label = self.df[self.df['Cell_Types'] == high_level_name].iloc[:, 2:].to_numpy() # Takes the two right-most columns, the numerical category value + the one hot encodings representing that category, and converts it to a np array
            length = _label.shape[1] # Should always return 7 since we have 6 classes and therefore 6 one hot encodings
